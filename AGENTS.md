...
configuration. Deploy tooling lives in `repo-tools/` so it is not published with
the JSON.
...
ppkg add windsurf
# or: deno run -A --reload jsr:@patdx/pkg@0.8.0 add windsurf
...
Live URLs are under `https://repo.pmil.me/...` (no `/api` prefix). HTML pages
are generated by `deno task gen-site` (`/` and `/package/<name>`), linking
shared `/site.css` and `/site.js`. The CLI resolves them from
builtin/local/remote sources and only executes named URL providers shipped in
`@patdx/pkg`. Deploy with Deno from `repo-tools/` (`deno task deploy`, runs
gen-site first).
